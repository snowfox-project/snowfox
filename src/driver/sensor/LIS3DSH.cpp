...
#include <spectre/driver/sensor/LIS3DSH.h>
...
LIS3DSH::LIS3DSH(uint8_t const i2c_address, hal::interface::I2CMaster & i2c_master)
...
bool LIS3DSH::readSingleRegister(uint8_t const reg_addr, uint8_t * data)
{
  return readMultipleRegister(reg_addr, data, 1);
}

bool LIS3DSH::writeSingleRegister(uint8_t const reg_addr, uint8_t const data)
{
...
bool LIS3DSH::readMultipleRegister(uint8_t const reg_addr, uint8_t * data, uint16_t const num_bytes)
{
  if(!_i2c_master.begin      (_i2c_address, false          )) return false;
  if(!_i2c_master.write      (reg_addr                     )) return false;
