...
{
  ssize_t const num_frames = num_bytes / sizeof(util::type::CanFrame);
  ...
  for(ssize_t f = 0;
      (f < num_frames);
      f++, bytes_read += sizeof(util::type::CanFrame))
  {
    util::type::CanFrame can_frame;

    if(!_control.receive(&can_frame)) break;

    memcpy(buffer + bytes_read, &can_frame, sizeof(util::type::CanFrame));
  }
...
ssize_t Can::write(uint8_t const * buffer, ssize_t const num_bytes)
{
  ...
  for(ssize_t f = 0;
      (f < num_frames);
      f++, bytes_written += sizeof(util::type::CanFrame))
  {
    util::type::CanFrame can_frame;
    memcpy(&can_frame, buffer + bytes_written, sizeof(util::type::CanFrame));
